...
// Experiment only: synchronous uint32 Karatsuba square with exact 96-bit leaf coefficients.
internal sealed class Limb32Square : IDisposable
{
    private uint[] arena = [];
    ...
    internal Limb32Square(string backend,int leafSize)
    {
        if(leafSize<4 || leafSize>1024)throw new ArgumentOutOfRangeException(nameof(leafSize));
        leaf=leafSize;
        Backend=backend=="avx512" && Avx512F.IsSupported ? "avx512" : backend!="scalar" && Avx2.IsSupported ? "avx2" : "scalar";
