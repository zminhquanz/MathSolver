...
internal static class SingleThreadBigIntegerPower
{
    ...
    // Custom arithmetic in Debug was much slower than the optimized runtime
    // BigInteger implementation. Keep the existing AVX2 prefix, but do not
    // replace large runtime squares in unoptimized builds.
    internal static bool IsLargeSquareBackendEnabled =>
#if DEBUG
        false;
#else
        true;
    ...
    internal static bool ShouldUseLargeSquareBackend(ulong magnitude, int exponent) =>
        IsLargeSquareBackendEnabled &&
        exponent >= 1_000_000 &&
        magnitude > 1 &&
        // Bound the whole power as well as each square. The old per-square
        // bound still admitted unmeasured, much larger final operands/batches.
        Math.Log2(magnitude) * exponent <= 2d * 32 * SimdBigIntegerSquare.MaximumWords;
        ...
        bool useAvx512 = false)
    {
        ...
        // Enable the large-square backend only within the measured workload range.
        // Hardware mode is captured once; workers and NTT are never created here.
        using var largeSquareWorkspace = useSimd && IsSupported && ShouldUseLargeSquareBackend(magnitude, exponent)
