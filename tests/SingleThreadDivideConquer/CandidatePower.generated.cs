...
/// </summary>
internal static class CandidatePower
{
        ...
        bool useSimd)
    {
        ...
        using var squareWorkspace = new CandidateSquare(CandidateSquare.Limit);
        ...
        ulong[] accumulatorWorkspace =
            useCustomWindow ? new ulong[(CandidateSquare.Limit * 2 + 8)] : [];
             ...
             bitIndex >= 0;)
        {
            ...
            if (!runtimeBigIntegerMode &&
                CandidateSquare.CanSquare(
                    resultMagnitude.Length))
            {
                resultMagnitude =
                    squareWorkspace.Square(
                        resultMagnitude,
    ...
    private static bool ShouldStartRuntimeBatching(int length, int exponent, int bitIndex) =>
        !CandidateSquare.CanSquare(length) ||
        (exponent >= RuntimeWindowOptimizationMinimumExponent &&
         length > CandidateSquare.Limit &&
