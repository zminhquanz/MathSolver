...
foreach (var mode in CalculationAccelerationManager.AvailableSelectableModes)
{
    ...
    if (mode == CalculationSimdMode.Sse) Check(CalculationAccelerationManager.UsePowerNttSse == Sse2.IsSupported, "SSE NTT fallback");
...
Check(!CalculationAccelerationManager.AllowAvx && !CalculationAccelerationManager.AllowAvx512 && !CalculationAccelerationManager.UsePowerNttAvx2 && !CalculationAccelerationManager.UsePowerNttSse && !CalculationAccelerationManager.UsePowerExportSimd, "Disabled gates all kernels");
