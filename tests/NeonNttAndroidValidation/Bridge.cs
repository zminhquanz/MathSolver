...
{
    internal static int ValidateNeonKernels()
    {
        int checks = 0;
        ...
        foreach (uint p in new[] { FirstModulus, SecondModulus })
        {
            ...
            for (int i = 0; i < 10000; i++)
            {
                ...
                ExecuteCachedGroupNeonPortable(product, p, rootsForProduct, shoup, 0, 4, 0, true);
                uint[] sumDifference = [a[0], a[1], a[2], a[3], b[0], b[1], b[2], b[3]];
                uint oneShoup = (uint)((1UL << 32) / p);
                ExecuteCachedGroupNeonPortable(sumDifference, p, [1, 1, 1, 1],
                    [oneShoup, oneShoup, oneShoup, oneShoup], 0, 4, 0, false);
                if (AdvSimd.Arm64.IsSupported)
                {
                    uint[] directProduct = [0, 0, 0, 0, a[0], a[1], a[2], a[3]];
                    ExecuteCachedGroupNeon(directProduct, p, rootsForProduct, shoup, 0, 4, 0, true);
                    uint[] directButterfly = [a[0], a[1], a[2], a[3], b[0], b[1], b[2], b[3]];
                    ExecuteCachedGroupNeon(directButterfly, p, [1, 1, 1, 1],
                        [oneShoup, oneShoup, oneShoup, oneShoup], 0, 4, 0, false);
                    if (!directProduct.SequenceEqual(product) || !directButterfly.SequenceEqual(sumDifference))
                        throw new Exception("AdvSimd / Vector128 mismatch");
                }
            ...
            using var plans = new SharedNttTwiddlePlans(roots, false, useNeonNtt: true);
            ...
            foreach (int length in new[] { 2, 4, 8, 16, 32, 256, 2048, 16384, 131072 })
            {
                ...
                ExecuteCachedTilesNeon(actual, p, team, plan, length, Math.Min(length, 16384), Math.Min(length, 2048), false, default);
                if (!actual.SequenceEqual(expected)) throw new Exception($"Forward {p} {length}");
                ExecuteCachedTilesNeon(actual, p, team, plan, length, Math.Min(length, 16384), Math.Min(length, 2048), true, default);
