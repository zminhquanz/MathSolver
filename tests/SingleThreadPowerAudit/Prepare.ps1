...
$controller = [IO.File]::ReadAllText((Join-Path $root 'MathSolver/Numerics/SingleThreadBigIntegerPower.cs')).Replace("`r`n", "`n")
$kernel = [IO.File]::ReadAllText((Join-Path $root 'MathSolver/Numerics/Avx2BigIntegerPower.cs')).Replace("`r`n", "`n")
$original = & git -c "safe.directory=$($root.Replace('\','/'))" show "${BaselineRevision}:MathSolver/Numerics/Avx2BigIntegerPower.cs"
